...
export function StaffPage() {
  ...
  async function handleSaveStaff() {
    ...
    // Password wajib untuk staff baru; opsional saat edit (kosong = tidak diubah)
    if (!editingStaff && (!formData.password || !isStrongPassword(formData.password))) {
      setFormError('Password wajib min 8 karakter, harus ada huruf, angka, dan simbol (!@#$ etc).')
      return
    }
    if (editingStaff && formData.password && !isStrongPassword(formData.password)) {
      setFormError('Password baru min 8 karakter, harus ada huruf, angka, dan simbol (!@#$ etc).')
      return
    }
    ...
    setSaving(true)
    try {
      await upsertStaff(staffData, !editingStaff)
      setIsFormOpen(false)
      setFormError('')
      showToast(editingStaff ? 'Data staff tersimpan di server!' : 'Staff baru tersimpan di server!', 'success')
    } catch (e) {
      setFormError(e instanceof Error ? e.message : 'Gagal menyimpan staff')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleConfirmToggleActive() {
    if (!toggleStaff || saving) return
    setSaving(true)
    try {
      await upsertStaff({ ...toggleStaff, active: !toggleStaff.active, password: '' }, false)
      setToggleStaff(null)
      showToast(`Staff ${toggleStaff.active ? 'dinonaktifkan' : 'diaktifkan'}!`, 'success')
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'Gagal mengubah status staff', 'error')
      setToggleStaff(null)
    } finally {
      setSaving(false)
    }
  ...
  async function handleDeleteStaff() {
    if (!deletingStaff || saving) return
    setSaving(true)
    try {
      await removeStaff(deletingStaff.id)
      setDeletingStaff(null)
      showToast('Staff dinonaktifkan di server.', 'success')
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'Gagal menghapus staff', 'error')
      setDeletingStaff(null)
    } finally {
      setSaving(false)
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-[16px] bg-white p-6 shadow-2xl space-y-4">
            ...
            <div className="space-y-3">
              ...
              <Field label={editingStaff ? 'Password Baru (kosongkan bila tidak diubah)' : 'Password'}>
                <div className="relative">
                  <input
                    type={showPassword ? 'text' : 'password'}
                    value={formData.password}
                    onChange={(e) => setFormData({ ...formData, password: e.target.value })}
                    placeholder={editingStaff ? 'Kosongkan bila tidak diubah' : 'Min 8: huruf, angka, simbol (contoh: Kasir123!)'}
            ...
            <div className="mt-6 flex justify-end gap-2 border-t border-sand pt-4">
              ...
              <Button
                className="flex-1"
                disabled={!formData.name.trim() || !formData.email.trim() || (!editingStaff && !formData.password) || saving}
      ...
      {toggleStaff && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-sm rounded-[16px] bg-white p-6 shadow-2xl">
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {saving ? 'Menyimpan…' : `Ya, ${toggleStaff.active ? 'Nonaktifkan' : 'Aktifkan'}`}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
            ...
            <div className="flex gap-2 justify-end">
                ...
                type="button"
                disabled={saving}
                className="rounded-lg bg-[#ba1a1a] px-4 py-2 text-xs font-semibold text-white hover:bg-[#ba1a1a]/90 transition-colors disabled:opacity-60"
